...
# 크롬 드라이버로 크롬을 실행한다.
# 사전
# 1. https://sites.google.com/a/chromium.org/chromedriver/downloads 에서 PC에 설치된 chrome browser와 맞는 버전의 드라이브 다운로드
# 2. path에 해당 chromedriver 실행파일 복사 -> pythone.exe가 있는 C:\ProgramData\Anaconda3 에 복사함
...
try:
    ...
    ws.append(["업무", "공고번호-차수", "공고번호-링크", "분류", "공고명", "공고명-링크", "공고기관", "수요기관", "계약방법", "입력일시"])
    for raw in result:
        ws.append(raw)
