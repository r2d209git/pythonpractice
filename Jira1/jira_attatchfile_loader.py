...
jira_options = {'server': "https://jira.skbroadband.com/"}
jira = JIRA(options=jira_options, basic_auth=('yk1.bae@sk.com', "accessToken"))

#Directory to save extracted attachments
dir_to_save = 'D:\\jira-plugin\\CV'

#Excel workbook for generation summary for attachments with Hyperlinks to open the attachments
wb = Workbook()
ws = wb.active
key_row = 1
parent_key_row = 1
...
jql_query = 'project = REC AND issuetype in (Epic, Story, Sub-task) AND "Epic Link" not in (REC-XX, REX-XX) AND Sprint in openSprints()'
stories_in_project = jira.seach_issues(jql_query, fields='*all')
